...
| | | | | The program is unsigned: with Secure Boot enforcing, `skillfish-coreunlock-efi` refuses to install it and the in-OS service keeps doing the unlock (two boots). The BC-250 firmware has no Secure Boot. LUKS is untouched: the program runs before GRUB and never reads a disk; TPM-bound unlocking (which we do not use) would just see one more measured binary, the same one on every boot |
