...
--
-- Hardware model:
--   * native ACP and the hidden A52 backend both ultimately need the one
...
-- Historical setting name kept for v0.5-v0.7 compatibility. It is the generic
-- encoded-hardware lock; v0.8-v0.13 also used it for E-AC-3.
...
local function create_encoded_bridge (gen, mode, frontend, backend_name)
  ...
  Core.sync (function (err)
    ...
    local function finish_ready ()
      log:notice ("AC3 mode READY: bc250_ac3 -> A52 640 kbps -> hw:Generic,3")
...
local function create_ac3_backend (gen)
  ...
  log:notice ("creating hidden A52 backend on " .. A52_PATH .. " (640 kbps)")

  local properties = {
    ["factory.name"] = "api.alsa.pcm.sink",
    ["node.name"] = AC3_BACKEND_NAME,
    ["node.description"] = "BC-250 AC3 640 kbps hardware backend",
...
local function stop_encoded_then (gen, next_mode, next_native_name)
  ...
  Core.sync (function (err)
    ...
    Core.sync (function (destroy_err)
      ...
      local function start_release_guard ()
        log:notice (old_label .. " backend graph gone; starting " ..
            tostring (SWITCH_DELAY_MS) ..
            " ms hardware guard after release acknowledgement")
        ...
        timer_source = Core.timeout_add (SWITCH_DELAY_MS, function ()
          ...
          set_encoded_hardware_lock (false,
              "encoded backend released, acknowledged and guard elapsed")
...
local function refresh_native_during_encoded (gen)
  ...
  Core.sync (function (err)
    ...
    Core.sync (function (destroy_err)
      ...
      local function start_hotplug_release_guard ()
        cancel_timer ()
        log:notice ("hotplug reprobe: encoded backend graph gone; starting " ..
            tostring (SWITCH_DELAY_MS) ..
            " ms release guard after release acknowledgement")
...
reconcile = function ()
  ...
  if is_encoded_mode (desired_mode) then
    ...
    if encoded_backend_present () then
      -- Tear down the old owner, keep native locked out, then create the
      -- replacement after one hardware release guard.
...
local function refresh_desired_from_authority (force)
  ...
  -- SkillFishOS: an encoded output is never chosen on its own. With the
  -- display in standby the native sink is missing, WirePlumber falls back to
  -- the always-present AC-3 frontend, and a monitor with speakers then gets
  -- the raw bitstream as PCM: a loud continuous noise. Only an explicit choice
  -- (default.configured.audio.sink) may take the hardware.
  if is_encoded_mode (mode) and authority ~= "configured" then
    log:notice ("automatic fallback landed on " .. tostring (name) ..
        "; ignored: an encoded output must be chosen by hand")
    mode = "other"
    native_name = nil
  end
...
nodes_om:connect ("object-added", function (om, node)
  local name = node.properties["node.name"]
  if name == AC3_FRONTEND or starts_with (name, NATIVE_PREFIX) then
