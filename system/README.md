...
| `etc/default/grub` | kernel cmdline (mitigations=off, split_lock_detect=off, ttm.pages_limit for the GTT). No `video=DP-1:e` — the DP fix is the `skillfish-dp-hotswap` service; no `bc250_cc_write_mode` — the 40 CUs are routed live by a service. |
