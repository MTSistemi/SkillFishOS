...
Presentation {
    ...
    property var s3: {
        "en": "<h2 style='color:#d8a849'>Eight cores, not six</h2><h3>The two the board hides</h3><p>The BC-250 ships as 6 cores / 12 threads. The missing two are switched off by configuration, not by defect: SkillFishOS can switch them on from the Tuner. <b>+20% measured</b> on multi-threaded work.</p>",
        "it": "<h2 style='color:#d8a849'>Otto core, non sei</h2><h3>I due che la scheda nasconde</h3><p>La BC-250 si presenta come 6 core / 12 thread. I due mancanti sono spenti dalla configurazione, non difettosi: SkillFishOS li accende dal Tuner, quando vuoi. <b>+20% misurato</b> nei carichi multi-thread.</p>",
        "pl": "<h2 style='color:#d8a849'>Osiem rdzeni, nie sześć</h2><h3>Te dwa, które płyta ukrywa</h3><p>BC-250 zgłasza się jako 6 rdzeni / 12 wątków. Brakujące dwa są wyłączone przez konfigurację, a nie uszkodzone: SkillFishOS włączy je z poziomu Tunera. <b>+20% zmierzone</b> w zadaniach wielowątkowych.</p>",
        "uk": "<h2 style='color:#d8a849'>Вісім ядер, а не шість</h2><h3>Ті два, які плата приховує</h3><p>BC-250 представляється як 6 ядер / 12 потоків. Двох бракує через конфігурацію, а не через дефект: SkillFishOS ввімкне їх із Tuner. <b>+20% виміряно</b> в багатопотокових задачах.</p>",
        "ru": "<h2 style='color:#d8a849'>Восемь ядер, а не шесть</h2><h3>Те два, которые плата прячет</h3><p>BC-250 представляется как 6 ядер / 12 потоков. Двух не хватает из-за конфигурации, а не из-за дефекта: SkillFishOS включит их из Tuner. <b>+20% измерено</b> в многопоточных задачах.</p>",
        "es": "<h2 style='color:#d8a849'>Ocho núcleos, no seis</h2><h3>Los dos que la placa esconde</h3><p>La BC-250 se presenta como 6 núcleos / 12 hilos. Los dos que faltan están apagados por configuración, no por defecto de fábrica: SkillFishOS los enciende desde el Tuner. <b>+20% medido</b> en cargas multihilo.</p>",
        "pt": "<h2 style='color:#d8a849'>Oito núcleos, não seis</h2><h3>Os dois que a placa esconde</h3><p>A BC-250 se apresenta como 6 núcleos / 12 threads. Os dois que faltam estão desligados por configuração, não por defeito: o SkillFishOS os liga pelo Tuner. <b>+20% medido</b> em cargas multithread.</p>",
        "de": "<h2 style='color:#d8a849'>Acht Kerne, nicht sechs</h2><h3>Die zwei, die die Platine verbirgt</h3><p>Die BC-250 meldet sich mit 6 Kernen / 12 Threads. Die fehlenden zwei sind durch die Einrichtung abgeschaltet, nicht defekt: SkillFishOS schaltet sie im Tuner ein. <b>+20% gemessen</b> bei Last über viele Threads.</p>",
        "fr": "<h2 style='color:#d8a849'>Huit cœurs, pas six</h2><h3>Les deux que la carte cache</h3><p>La BC-250 se présente avec 6 cœurs / 12 fils. Les deux qui manquent sont éteints par la configuration, pas défectueux : SkillFishOS les allume depuis le Tuner. <b>+20% mesuré</b> sur les charges multifils.</p>"
    }
    property var s4: {
