...
T["pl"] = dict(
    ...
    voci=[("Stan", "jedna liczba na kartę: zegar i sufit GPU, CPU, jednostki obliczeniowe, wentylator, jądro, governor, sterownik Vulkan, czy ostatni start nastąpił po czystym zamknięciu. Tu niczego się nie ustawia."),
          ...
          ("AI", "Unsloth Studio na Vulkanie: wł./wył., sprzęt, limit GTT."),
...
- **VRAM**: podział UMA w CMOS, stosowany przy następnym starcie. Przy dynamicznych 512 MB niektóre gry wybierają niskie tekstury: jeśli są rozmyte, spróbuj 4 lub 6 GB na stałe.
...
T["uk"] = dict(
    ...
    voci=[("Стан", "одне число на картку: частота і стеля GPU, CPU, обчислювальні блоки, вентилятор, ядро, governor, драйвер Vulkan, чи було останнє завантаження після чистого вимкнення. Тут нічого не налаштовується."),
          ...
          ("ШІ", "Unsloth Studio на Vulkan: увімк./вимк., обладнання, ліміт GTT."),
...
- **VRAM**: розподіл UMA в CMOS, застосовується при наступному завантаженні. З динамічними 512 МБ деякі ігри обирають низькі текстури: якщо вони розмиті, спробуйте 4 або 6 ГБ фіксовано.
...
for lingua, d in T.items():
    ...
    testo = ("---\ntitle: %s\ndescription: %s\ngroup: %s\norder: 4\n---\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s\n"
             % (q(d["title"]), q(d["desc"]), GRUPPO[lingua], d["intro"], d["sezioni"], voci,
                d["tuner_t"], d["tuner"], d["monitor_t"], d["monitor"], d["giochi_t"], d["giochi"], d["fine_t"], d["fine"]))
