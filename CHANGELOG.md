...
### Fixed
- **The installer no longer reboots the board mid-installation** (#31) — `skillfish-core-unlock` ran at `sysinit.target` on every boot, including inside the live session, and deliberately reboots after writing the SMU core-presence mask. On real hardware the board restarted seconds after start-up and, if that landed while Calamares was copying, the installation died. Reported by a user on 26.06.3; both reported symptoms ("installer crashes", "board reboots once at first boot") were the same cause. **The 8-core unlock is now off by default and opt-in** via a switch in SkillFishOS Tuner, guarded by `ConditionKernelCommandLine=!boot=live` and a marker file. Upgrading preserves the unlock for anyone already using it. ⚠️ It was never caught because every install test ran in a VM, where the SMU write fails and the reboot never happens.
- **Btrfs installs boot** (#12, reopened) — the real cause: Calamares copies with `rsync -aHAXS`, and that `S` is `--sparse`, so the 512 zero bytes at the end of `vmlinuz` became a hole. On Btrfs a hole has no extent, the map ends one block short of the declared size with no `eof` flag, and GRUB stops with "premature end of file". The 26.06.3 repair rewrote the files with `cp --reflink=never` but **without** `--sparse=never`, and GNU `cp` reproduces the hole faithfully — so it repaired nothing for six days. The CI guard meant to catch this searched for a string present in the broken version too, and passed either way. The repair now verifies its own result.
- **SSH works on a fresh install** — host keys are stripped from the image on purpose, but `skillfish-sshd-keygen` was not enabled, so `sshd` failed forever with "no hostkeys available". Five other services were in the same state (core unlock, freeze check, GPU clock sampler, Wake-on-LAN, first-boot snapshots): `systemctl enable` from a package postinst does not take effect in the live-build chroot. They are now enabled by a hook that **verifies the symlink exists** and fails the build otherwise.
- **A third-party sensor module can no longer abort the installation** — `nct6687d` failed to build, `dpkg-reconfigure` returned non-zero and Calamares gave up *before* writing the bootloader. That step now tolerates the failure but still demands an initrd.
- **The `nct6687` module builds** (#30) — its `Kbuild` file, which is where the upstream project keeps `obj-m`, was never copied. Without it `make` finds nothing to compile and exits **successfully** while DKMS reports failure. The hook now also unregisters the module if no `.ko` is produced, so a broken module cannot make every future kernel upgrade fail.
- **The initrd is zstd-compressed**, not gzip — the `zstd` binary was missing from the image.
- **Third-party code without a licence is no longer shipped** — `bc250_memcfg` and `bc250-cu-ref` are excluded from the images; the memory-split tool is our own `skillfish-memcfg` (GPL-3.0).

### Changed
- The live session of the live-build image: X11 instead of Wayland, no screen lock (it asked for a password nobody could know), and an *Install SkillFishOS* icon on the desktop. All three came from one wrong assumption — that the live user is called `live`, when live-build names it `user`.
- The ISO boot menu carries SkillFishOS branding instead of "Debian GNU/Linux none (sid)".
