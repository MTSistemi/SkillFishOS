...
- **On-device AI** — **Unsloth Studio** accelerated in **Vulkan** on the integrated GPU (**5.1×** faster than CPU, measured), serving both a chat UI and an OpenAI-compatible API, with a one-click switch in the Control Center's **AI** section (mirrored in the Remote Manager) that frees the GPU when you want to play. Models are GGUF pulled straight from Hugging Face.
- **Native PyQt6 apps, grouped under a dedicated "SkillFishOS" menu** — the **Control Center** above is the single window for tuning, monitoring and maintenance; three apps stay separate from it: **Hub**, a Discover-style software centre (browse by category, search, read app pages with screenshots, and install/remove/update across **APT**, incl. the signed [`aetherium`](https://mtsistemi.github.io/SkillFishOS/) repo, **Flatpak** and **Snap**, plus add/remove software sources); the **HUD**, a translucent on-screen overlay with live sensor readouts (BC-250 only); and the **Remote Manager**, the web dashboard described below. Every app ships as an updatable `.deb` — including **`skillfish-base`** (hardware **watchdog** + boot-time **freeze detector**: the board recovers from hard hangs by itself and warns you if your profile is unstable) and **`skillfish-console`** (a SteamOS-style **"SkillFishOS Console (Big Picture)"** session on the login screen, now also a Control Center section).
...
| About this system | SkillFishOS Control Center — Status |
|---|---|
| ![About](https://raw.githubusercontent.com/MTSistemi/SkillFishOS/main/screenshots/about.jpg) | ![Control Center — Status](https://raw.githubusercontent.com/MTSistemi/SkillFishOS/main/screenshots/control-center-stato.png) |

| Control Center — Tuner | EmuDeck — easy emulation |
|---|---|
| ![Control Center — Tuner](https://raw.githubusercontent.com/MTSistemi/SkillFishOS/main/screenshots/control-center-tuner.png) | ![EmuDeck](https://raw.githubusercontent.com/MTSistemi/SkillFishOS/main/screenshots/emudeck.jpg) |

---

## Performance

> All measured on **our own BC-250** with SkillFishOS at **1080p** (40 CUs unlocked, kernel 7.1.7-skillfishos, Mesa 26.0.8 — 7.0.11 and 7.1.7 measure within ±2% of each other; we ship 7.2.5 today and have not re-measured these frame rates against it). Full per-benchmark detail — every setting, clock, voltage, temperature and power reading — is on the **[Performance & benchmarks page →](https://skillfishos.com/docs/prestazioni/)**
...
Prebuilt kernel `.deb`s (two flavours) are published under [**Releases**](../../releases/tag/kernel-7.2.5-skillfishos):

```sh
# main BC-250 kernel (znver2)
sudo dpkg -i linux-image-7.2.5-skillfishos_7.2.5-1_amd64.deb
# …or x64 (any other PC/VM): linux-image-7.2.5-skillfishos-x64_7.2.5-1_amd64.deb
```
...
docs/              full documentation
  ...
  DESKTOP.md         KDE Plasma, steampunk theme, HUD, Control Center
...
- **[docs/DESKTOP.md](docs/DESKTOP.md)** — KDE Plasma, the steampunk theme and the live HUD.
- **[docs/CONTROL-CENTER.md](docs/CONTROL-CENTER.md)** — the one window: Tuner, Fan, Monitor, Games, Profiles, Kernel, Snapshots, AI, Emulators, Console, ISO.
