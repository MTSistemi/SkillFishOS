...
> **Warning:** **Silicon lottery + cooling.** The numbers hold for *this* chip with adequate cooling. The stock cooler is marginal: "back-to-back" comparisons without pauses are skewed by *heat-soak* — let the card cool a few minutes between runs.
...
| Kernel | **7.0.10-skillfishos** (linux-tkg) — the version these numbers were taken with; we ship **7.1.7** today, re-measured within 2% |
...
| Workload | 6c/12t | 8c/16t | |
|---|---|---|---|
| `xz -T` compression | 6.41 s | **5.11 s** | **+20%** |
| CPU LLM inference | 34.0 tok/s | **40.8 tok/s** | **+20%** |
| Temperature | 66 °C | 68 °C | +2 °C |

It's +20% rather than the theoretical +33%: memory bandwidth and thread overhead eat the difference. Still **a fifth more performance for free**.

### Overclocking with all 8 cores

Re-measured step by step, every rung **stable with 0 MCEs**:

| Target | Reached under load | Score | Temp | Fan |
|---|---|---|---|---|
| 3500 (control) | 3475 | 5118 ev/s | 57 °C | — |
| 3700 | 3673 | 5410 | 62 °C | 50% |
| 3900 | 3872 | 5704 | 71 °C | 68% |
| **4000** | **3971** | **5849** | **81 °C** | **93%** |

**Stable maximum: 4000 MHz**, +14% on the score over 3500 — only reachable once the fan control was fixed. **Warning:** Under a **combined CPU+GPU** load the clock settles at 3375–3492 MHz at 86 °C: past ~3900 the limiter is the heatsink, not the silicon.

---

## Thermal validation (stress test)
...
| **CPU stress** (12 threads, 120 s) | CPU **3.68–3.69 GHz** | k10 **85 °C** (at the cap) | historical figure, taken **before** the 8-core unlock |
