---
title: Sources and references
description: All the sources, upstream projects and tools SkillFishOS is built on.
group: Reference
order: 6
---

SkillFishOS is an integration effort: it brings together the work of many open source communities and projects. This page collects the sources cited in the documentation and the projects the system is built on.

## BC-250 hardware documentation

- **[bc250.info](https://bc250.info)** — community wiki about the BC-250 board.
- **[elektricm.github.io/amd-bc250-docs](https://elektricm.github.io/amd-bc250-docs)** — detailed technical documentation.

## BC-250-specific projects

- **[cyan-skillfish-governor (Magnap)](https://github.com/Magnap/cyan-skillfish-governor)** — SMU governor for GPU frequencies, used until September 2026: now replaced by our own `skillfish-vf-governor`.
- **[bc250_smu_oc (bc250-collective)](https://github.com/bc250-collective/bc250_smu_oc)** — overclock/undervolt via the SMU.
- **[bc250-core-unlock (rw-r-r-0644)](https://github.com/rw-r-r-0644/bc250-core-unlock)** — unlocking the 8 CPU cores.
- **[bc250-40cu-unlock (duggasco)](https://github.com/duggasco/bc250-40cu-unlock)** — unlocking the 40 Compute Units.
- **[bc250_memcfg (fanoush)](https://github.com/fanoush/bc250_memcfg)** — memory configuration.
- **[bc250 (akandr)](https://github.com/akandr/bc250)** — setup guide for the board: Ollama and Vulkan inference, image generation.
- **[bc250-rocm (akandr)](https://github.com/akandr/bc250-rocm)** — ROCm/HIP on gfx1013, measured against Vulkan.

## Cooling and 3D-printable cases (free STL)

- **[Console Style Case (Arthrimus)](https://www.thingiverse.com/thing:7172528)** — case + PSU, 120 mm shroud.
- **[ASRock BC-250 Shell Case (onemorecap)](https://www.printables.com/model/1228207-asrock-amd-bc-250-shell-case)** — snap-on shell.
- **[Yet Another BC-250 Fan Shroud (ViRazY)](https://www.printables.com/model/1339540-yet-another-bc-250-fan-shroud)** — 140 + 120 mm.
- **[Case ATX PSU & Fan Duct (ZMASLO)](https://www.printables.com/model/1616167-amd-bc-250-case-atx-psu-fan-duct)** — duct + ATX PSU.
- **[ATX PSU case (CatSiewDai)](https://www.thingiverse.com/thing:7269520)** — full case.
- **[OC vRAM Fan Kit (marccyberwiz)](https://www.thingiverse.com/thing:7271946)** — VRAM-dedicated fan.
- **[Cooling Solutions — amd-bc250-docs](https://elektricm.github.io/amd-bc250-docs/hardware/cooling/)** · **[mothenjoyer69/bc250-documentation](https://github.com/mothenjoyer69/bc250-documentation)** — cooling guides.

## System base

- **[Debian](https://www.debian.org/)** — the base distribution (*sid* branch).
- **[KDE Plasma](https://kde.org/plasma-desktop/)** — the desktop environment.
- **[linux-tkg (Frogging-Family)](https://github.com/Frogging-Family/linux-tkg)** — kernel build recipe.
- **[Mesa / RADV](https://docs.mesa3d.org/drivers/radv.html)** — open source Vulkan graphics drivers.
- **[amdgpu driver](https://docs.kernel.org/gpu/amdgpu/)** — AMD kernel driver.

## Storage and snapshots

- **[Btrfs](https://btrfs.readthedocs.io/)** · **[Snapper](http://snapper.io/)** · **[grub-btrfs (Antynea)](https://github.com/Antynea/grub-btrfs)**

## Desktop, theme and remote

- **[Kvantum](https://github.com/tsujan/Kvantum)** · **[Conky](https://github.com/brndnmtthws/conky)** · **[x11vnc](https://github.com/LibVNC/x11vnc)**
- **[PipeWire](https://pipewire.org/)** · **[SDDM](https://github.com/sddm/sddm)** · **[Plymouth](https://www.freedesktop.org/wiki/Software/Plymouth/)** · **[NetworkManager](https://networkmanager.dev/)**

## Gaming and emulation

- **[Steam](https://store.steampowered.com/)** · **[gamescope](https://github.com/ValveSoftware/gamescope)** · **[gamemode](https://github.com/FeralInteractive/gamemode)** · **[MangoHud](https://github.com/flightlessmango/MangoHud)**
- **[Heroic Games Launcher](https://heroicgameslauncher.com/)** · **[ProtonUp-Qt](https://github.com/DavidoTek/ProtonUp-Qt)** · **[Proton GE](https://github.com/GloriousEggroll/proton-ge-custom)**
- **[EmuDeck](https://www.emudeck.com/)** · **[ES-DE](https://es-de.org/)** · **[RetroArch](https://www.retroarch.com/)**
- **[Waydroid](https://waydro.id/)** · **[Sober](https://sober.vinegarhq.org/)** · **[OptiScaler](https://github.com/optiscaler/OptiScaler)**

## Local AI

- **[Unsloth](https://unsloth.ai/)** · **[llama.cpp](https://github.com/ggml-org/llama.cpp)** · **[Hugging Face](https://huggingface.co/)**

## Tools and infrastructure

- **[Calamares](https://calamares.io/)** — installer.
- **[balenaEtcher](https://etcher.balena.io/)** · **[Ventoy](https://www.ventoy.net/)** — ISO writing.
- **[reprepro](https://salsa.debian.org/debian/reprepro)** — APT repository.
- **[Flatpak](https://flatpak.org/)** — application packaging.
- **[vkpeak](https://github.com/nihui/vkpeak)** · **[clpeak](https://github.com/krrishnarraj/clpeak)** · **[sysbench](https://github.com/akopytov/sysbench)** — benchmarks.

## The project

- **Code and documentation** — [github.com/MTSistemi/SkillFishOS](https://github.com/MTSistemi/SkillFishOS)
- **Website** — [skillfishos.com](https://skillfishos.com)

---

> SkillFishOS is not affiliated with AMD. "AMD", "Ryzen", "RDNA" and related trademarks belong to Advanced Micro Devices, Inc. All other trademarks mentioned belong to their respective owners.
