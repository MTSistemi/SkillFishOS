...
- **Native SkillFishOS apps** (grouped under the **"SkillFishOS"** menu, installable/updatable as a `.deb` from the signed repo): the **[Control Center](/en/docs/control-center)** brings together in one window Tuner (BC-250 overclock/undervolt/fan/CU control), Fan, Monitor (live temperature/frequency/voltage/fan charts), Games, Profiles, Kernel (pick the boot kernel and uninstall old ones), Snapshots, AI (on-demand local LLM on the integrated GPU), Emulators, Console (the SteamOS-style "Big Picture" session, also from the login screen) and ISO; the **Hub** stays separate as the Discover-style software centre (APT + Flatpak + Snap) with app pages, a screenshot carousel and software-source management; **Base** is the hardware watchdog and freeze detector with desktop notification, always running in the background.
