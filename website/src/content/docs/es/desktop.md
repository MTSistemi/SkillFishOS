...
- **Aplicaciones propias de SkillFishOS** (agrupadas en el menú **«SkillFishOS»**, instalables y actualizables como `.deb` desde el repositorio firmado): el **[Control Center](/es/docs/control-center)** reúne en una sola ventana Tuner (control de overclock, undervolt, ventilador y CU de la BC-250), Ventilador, Monitor (gráficas en vivo de temperatura, frecuencia, voltaje y ventilador), Juegos, Perfiles, Kernel (elegir el núcleo de arranque y desinstalar los viejos), Instantáneas, AI (modelo de lenguaje local en la GPU integrada, bajo demanda), Emuladores, Console (la sesión «Big Picture» al estilo SteamOS, también desde la pantalla de acceso) e ISO; el **Hub** sigue aparte como centro de software al estilo Discover (APT + Flatpak + Snap) con páginas de aplicación, carrusel de capturas y gestión de las fuentes; **Base** es el vigilante por hardware y detector de cuelgues con aviso en el escritorio, siempre activo en segundo plano.
