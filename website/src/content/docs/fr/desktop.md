...
- **Applications natives de SkillFishOS** (regroupées sous le menu **« SkillFishOS »**, installables et actualisables en `.deb` depuis le dépôt signé) : le **[Control Center](/fr/docs/control-center)** réunit dans une seule fenêtre le Tuner (overclock, undervolt, ventilateur et CU de la BC-250), le Ventilateur, le Monitor (courbes de température, fréquence, tension et ventilateur en direct), les Jeux, les Profils, le Kernel (choisir le noyau de démarrage et désinstaller les anciens), les Instantanés, l'IA (LLM local à la demande sur le GPU intégré), les Émulateurs, la Console (la session « Big Picture » façon SteamOS, aussi depuis l'écran de connexion) et l'ISO ; le **Hub** reste à part comme logithèque façon Discover (APT + Flatpak + Snap) avec fiches d'applications, carrousel de captures et gestion des sources ; **Base** est le chien de garde matériel et le détecteur de blocage avec notification, toujours actif en arrière-plan.
