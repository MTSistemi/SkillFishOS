...
- **Aplicativos próprios do SkillFishOS** (reunidos no menu **“SkillFishOS”**, instaláveis e atualizáveis como `.deb` pelo repositório assinado): o **[Control Center](/pt/docs/control-center)** reúne numa janela só o Tuner (controle de overclock, undervolt, ventoinha e CU da BC-250), Ventoinha, Monitor (gráficos ao vivo de temperatura, frequência, tensão e ventoinha), Jogos, Perfis, Kernel (escolher o kernel de boot e desinstalar os antigos), Snapshots, AI (modelo de linguagem local na GPU integrada, quando você quiser), Emuladores, Console (a sessão “Big Picture” no estilo SteamOS, também pela tela de acesso) e ISO; o **Hub** continua à parte como central de programas no estilo Discover (APT + Flatpak + Snap) com páginas de aplicativo, carrossel de capturas e gestão das fontes; o **Base** é o vigia por hardware e detector de travamentos com aviso na área de trabalho, sempre ativo em segundo plano.
