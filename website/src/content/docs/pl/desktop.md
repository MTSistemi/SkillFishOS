...
- **Własne aplikacje SkillFishOS** (zebrane w menu **„SkillFishOS”**, instalowane i aktualizowane jako `.deb` z podpisanego repozytorium): **[Control Center](/pl/docs/control-center)** łączy w jednym oknie Tuner (podkręcanie, napięcia, wentylator i CU dla BC-250), Wentylator, Monitor (wykresy temperatury, częstotliwości, napięcia i wentylatora na żywo), Gry, Profile, Kernel (wybór jądra do uruchomienia i usuwanie starych), Migawki, AI (lokalny model na zintegrowanej grafice, na żądanie), Emulatory, Console (sesja „Big Picture” w stylu SteamOS, także z ekranu logowania) i ISO; **Hub** zostaje osobno jako centrum oprogramowania w stylu Discovera (APT + Flatpak + Snap) ze stronami aplikacji, karuzelą zrzutów ekranu i zarządzaniem źródłami; **Base** to sprzętowy watchdog i wykrywacz zawieszeń z powiadomieniem na pulpicie, zawsze działający w tle.
