...
- **Eigene SkillFishOS-Anwendungen** (im Menü **„SkillFishOS“** zusammengefasst, als `.deb` aus der signierten Paketquelle installierbar und aktualisierbar): das **[Control Center](/de/docs/control-center)** bündelt in einem Fenster Tuner (Steuerung von Übertaktung, Undervolting, Lüfter und CU der BC-250), Lüfter, Monitor (Kurven in Echtzeit für Temperatur, Takt, Spannung und Lüfter), Spiele, Profile, Kernel (Startkernel wählen und alte entfernen), Schnappschüsse, KI (lokales Sprachmodell auf der eingebauten GPU, bei Bedarf), Emulatoren, Console (die „Big Picture“-Sitzung im Stil von SteamOS, auch vom Anmeldebildschirm aus) und ISO; der **Hub** bleibt als eigene Anwendungsverwaltung im Stil von Discover (APT + Flatpak + Snap) mit Anwendungsseiten, Bilderkarussell und Verwaltung der Quellen; **Base** ist der Hardware-Wachhund und Einfrier-Melder mit Meldung auf dem Schreibtisch, ständig im Hintergrund aktiv.
