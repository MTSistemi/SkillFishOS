...
SkillFishOS сделан так, чтобы терминал **не** требовался: для обычной работы хватает [Tuner](/ru/docs/control-center) и графических приложений. Эта страница — для тех, кто хочет **повозиться** или разобраться в неполадке. Привилегированные команды идут через `sudo`.
...
# состояние регулятора V/F видеоядра
systemctl status skillfish-vf-governor
cat /etc/skillfish-vf-governor.json                         # кривая МГц/мВ, потолок, пороги
# наблюдение за GPU в реальном времени
nvtop        # или: radeontop
```

> На BC-250 управление частотой идёт **не** через обычный sysfs amdgpu, а через **регулятор V/F** (`skillfish-vf-governor`). Меняйте значения из [Tuner](/ru/docs/control-center), а не руками.
...
Управлять CU удобнее из **сетки** в [Tuner](/ru/docs/control-center) (щелчок и наборы, с «Проверкой CU»). Первые 24 закреплены драйвером и включены всегда.
...
sudo btrfs subvolume list /          # подтома (@, @home, @log, @cache, @games)
