...
SkillFishOS è pensato per **non** richiedere il terminale: il [Tuner](/docs/control-center) e le app grafiche bastano per l'uso normale. Questa pagina è per chi vuole **smanettare** o diagnosticare. Comandi privilegiati = con `sudo`.
...
# stato del governor V/F della GPU
systemctl status skillfish-vf-governor
cat /etc/skillfish-vf-governor.json                         # curva MHz/mV, tetto, soglie
# monitoraggio GPU in tempo reale
nvtop        # oppure: radeontop
```

> Sulla BC-250 il controllo frequenze **non** passa dal sysfs amdgpu standard ma dal **governor V/F** (`skillfish-vf-governor`). Modifica i valori dal [Tuner](/docs/control-center), non a mano.
...
Le CU si gestiscono comodamente dalla **griglia** del [Tuner](/docs/control-center) (clic + preset, con «Test CU»). Le prime 24 sono bloccate dal driver e sempre attive.
...
```bash
# un solo servizio nativo: niente container, Docker non e' installato
systemctl status skillfish-unsloth   # stato del motore AI
sudo systemctl start skillfish-unsloth   # avvialo
sudo systemctl stop skillfish-unsloth    # fermalo e libera GPU/RAM
curl -s localhost:8888/              # l’interfaccia di Unsloth Studio (solo da locale)
```
...
sudo skillfish-rollback 12           # fai diventare lo snapshot 12 il sistema
sudo skillfish-rollback --annulla    # ci hai ripensato: rimetti quello di prima
sudo skillfish-rollback --pulisci    # butta i sistemi messi da parte dai ripristini

# dal menu GRUB → "SkillFishOS snapshots" entri nello stesso snapshot ma in
# SOLA LETTURA: serve a guardare e recuperare file, poi usi il comando qui sopra
```
