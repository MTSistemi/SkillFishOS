...
- **App native SkillFishOS** (raggruppate nel menu **"SkillFishOS"**, installabili/aggiornabili come `.deb` dal repo firmato): il **[Control Center](/docs/control-center)** riunisce in una finestra sola Tuner (overclock/undervolt/ventola/CU della BC-250), Ventola, Monitor (grafici live di temperatura/frequenza/tensione/ventola), Giochi, Profili, Kernel (scegli il kernel di avvio e disinstalla quelli vecchi), Snapshot, AI (LLM locale on-demand sulla GPU integrata), Emulatori, Console (la sessione "Big Picture" stile SteamOS, anche dalla schermata di accesso) e ISO; l'**Hub** resta a parte come centro software in stile Discover (APT + Flatpak + Snap) con pagine app, carosello di screenshot e gestione delle sorgenti; **Base** è il watchdog hardware e il rilevatore di blocchi con notifica, sempre attivo in background.
