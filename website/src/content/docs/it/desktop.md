...
- **App native SkillFishOS** (raggruppate nel menu **"SkillFishOS"**, ognuna installabile/aggiornabile come `.deb` dal repo firmato): **Tuner** (overclock/undervolt/ventola/CU della BC-250), **AI** (LLM locale on-demand sulla GPU integrata), **Monitor** (grafici live di temperatura/frequenza/tensione/ventola), **Kernel Manager** (scegli il kernel di avvio e disinstalla quelli vecchi), **ISO Mount**, **Hub** — il centro software in stile Discover (APT + Flatpak + Snap) con pagine app, carosello di screenshot e gestione delle sorgenti — più **Base** (watchdog hardware + rilevatore di blocchi con notifica) e **Console**, la sessione **"SkillFishOS Console (Big Picture)"** selezionabile dalla schermata di accesso, stile SteamOS.
