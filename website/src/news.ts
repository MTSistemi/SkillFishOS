...
export const news: Post[] = [
  {
    ...
    testo: {
      it: "Fino a ieri il pacchetto del kernel scaricava sempre la versione della BC-250, anche su un PC normale. Quel kernel è compilato per il processore della scheda e su un'altra macchina non parte: GRUB carica e poi più niente. È capitato davvero a chi ci ha provato. Da oggi il pacchetto guarda la macchina prima di scaricare, e su un PC qualunque prende una versione <strong>-x64</strong>, compilata per un processore generico e con i firmware e i driver che servono a un portatile. Nello stesso giro abbiamo tolto un difetto più silenzioso: i servizi che hanno senso solo sulla scheda si portano adesso il controllo dell'hardware dentro di sé, invece di riceverlo dallo script di installazione. Chi installava su un PC normale si ritrovava un servizio che falliva a ogni avvio senza che si capisse perché. L'abbiamo visto solo avviando davvero una macchina che non è una BC-250: compilare per lei non bastava.",
      en: "Until yesterday the kernel package always downloaded the BC-250 build, even on an ordinary PC. That kernel is compiled for the board's processor and does not boot anywhere else: GRUB loads and then nothing. It really happened to people who tried. From today the package looks at the machine before downloading, and on an ordinary PC it takes an <strong>-x64</strong> build, compiled for a generic processor and carrying the firmware and drivers a laptop needs. In the same round we removed a quieter fault: the services that only make sense on the board now carry their hardware check inside themselves, instead of receiving it from the install script. Anyone installing on an ordinary PC ended up with a service that failed at every boot with no obvious reason. We only saw it by actually booting a machine that is not a BC-250 — building for one was not enough.",
      pl: "Do wczoraj pakiet jądra pobierał zawsze wersję dla BC-250, nawet na zwykłym pececie. Tamto jądro jest skompilowane pod procesor płyty i nigdzie indziej nie startuje: GRUB się ładuje i nic więcej. Przydarzyło się to naprawdę tym, którzy spróbowali. Od dziś pakiet patrzy na maszynę, zanim pobierze, i na zwykłym pececie bierze wersję <strong>-x64</strong>, skompilowaną pod ogólny procesor, z firmware'em i sterownikami, których potrzebuje laptop. Przy tej samej okazji usunęliśmy cichszą usterkę: usługi, które mają sens tylko na płycie, noszą teraz sprawdzenie sprzętu w sobie, zamiast dostawać je od skryptu instalacyjnego. Kto instalował na zwykłym pececie, dostawał usługę padającą przy każdym uruchomieniu bez widocznego powodu. Zobaczyliśmy to dopiero, uruchamiając naprawdę maszynę, która nie jest BC-250 — budowanie dla niej nie wystarczyło.",
      uk: "До вчора пакунок ядра завжди завантажував збірку для BC-250, навіть на звичайному ПК. Те ядро зібране під процесор плати й більше ніде не стартує: GRUB вантажиться, а далі нічого. Це справді траплялося з тими, хто пробував. Від сьогодні пакунок дивиться на машину, перш ніж завантажувати, і на звичайному ПК бере збірку <strong>-x64</strong>, зібрану під загальний процесор, із мікропрограмами та драйверами, потрібними ноутбуку. У тому ж заході ми прибрали тихішу ваду: служби, які мають сенс лише на платі, тепер несуть перевірку обладнання в собі, а не отримують її від сценарію встановлення. Той, хто встановлював на звичайний ПК, отримував службу, яка падала за кожного завантаження без видимої причини. Ми побачили це лише тоді, коли справді запустили машину, яка не є BC-250: збирати для неї було замало.",
      ru: "До вчерашнего дня пакет ядра всегда скачивал сборку для BC-250, даже на обычном ПК. То ядро собрано под процессор платы и больше нигде не стартует: GRUB грузится, а дальше ничего. Это действительно случалось с теми, кто пробовал. С сегодняшнего дня пакет смотрит на машину, прежде чем скачивать, и на обычном ПК берёт сборку <strong>-x64</strong>, собранную под обычный процессор, с прошивками и драйверами, которые нужны ноутбуку. В том же заходе мы убрали более тихую неисправность: службы, которые имеют смысл только на плате, теперь несут проверку оборудования в себе, а не получают её от установочного сценария. Тот, кто ставил систему на обычный ПК, получал службу, падавшую при каждой загрузке без понятной причины. Мы увидели это, только по-настоящему запустив машину, которая не является BC-250: собирать для неё было мало.",
      es: "Hasta ayer el paquete del núcleo descargaba siempre la versión de la BC-250, incluso en un PC normal. Ese núcleo está compilado para el procesador de la placa y en otra máquina no arranca: GRUB carga y después nada. Le pasó de verdad a quien lo intentó. Desde hoy el paquete mira la máquina antes de descargar, y en un PC cualquiera coge una versión <strong>-x64</strong>, compilada para un procesador genérico y con los firmwares y los controladores que necesita un portátil. En la misma vuelta hemos quitado un fallo más silencioso: los servicios que solo tienen sentido en la placa llevan ahora la comprobación del hardware dentro de sí, en lugar de recibirla del script de instalación. Quien instalaba en un PC normal se encontraba un servicio que fallaba en cada arranque sin que se entendiera por qué. Lo vimos solo arrancando de verdad una máquina que no es una BC-250: compilar para ella no bastaba.",
      pt: "Até ontem o pacote do kernel descarregava sempre a versão da BC-250, mesmo num PC normal. Esse kernel é compilado para o processador da placa e noutra máquina não arranca: o GRUB carrega e depois nada. Aconteceu mesmo a quem tentou. A partir de hoje o pacote olha para a máquina antes de descarregar e, num PC qualquer, leva uma versão <strong>-x64</strong>, compilada para um processador genérico e com os firmwares e os controladores de que um portátil precisa. Na mesma volta tirámos uma falha mais silenciosa: os serviços que só fazem sentido na placa passam a levar a verificação do hardware dentro de si, em vez de a receberem do script de instalação. Quem instalava num PC normal ficava com um serviço que falhava em todos os arranques sem que se percebesse porquê. Só o vimos ao arrancar mesmo uma máquina que não é uma BC-250: compilar para ela não chegava.",
      de: "Bis gestern lud das Kernel-Paket immer den BC-250-Build herunter, auch auf einem gewöhnlichen PC. Dieser Kernel ist für den Prozessor der Platine übersetzt und startet anderswo nicht: GRUB lädt, und dann nichts mehr. Das ist Leuten, die es versucht haben, wirklich passiert. Ab heute schaut das Paket auf die Maschine, bevor es herunterlädt, und holt auf einem beliebigen PC einen <strong>-x64</strong>-Build, der für einen allgemeinen Prozessor übersetzt ist und die Firmware und Treiber mitbringt, die ein Notebook braucht. In derselben Runde haben wir einen leiseren Fehler beseitigt: Die Dienste, die nur auf der Platine Sinn ergeben, tragen ihre Hardware-Prüfung jetzt in sich, statt sie vom Installationsskript zu bekommen. Wer auf einem gewöhnlichen PC installierte, bekam einen Dienst, der bei jedem Start ohne erkennbaren Grund scheiterte. Gesehen haben wir das erst, als wir eine Maschine, die keine BC-250 ist, wirklich gestartet haben — für sie zu bauen reichte nicht.",
      fr: "Jusqu'à hier, le paquet du noyau téléchargeait toujours la version de la BC-250, même sur un PC ordinaire. Ce noyau est compilé pour le processeur de la carte et ne démarre nulle part ailleurs : GRUB charge, puis plus rien. C'est réellement arrivé à ceux qui ont essayé. À partir d'aujourd'hui, le paquet regarde la machine avant de télécharger et, sur un PC quelconque, prend une version <strong>-x64</strong>, compilée pour un processeur générique et pourvue des micrologiciels et des pilotes dont un portable a besoin. Dans la même passe, nous avons supprimé un défaut plus discret : les services qui n'ont de sens que sur la carte portent désormais leur test matériel en eux-mêmes, au lieu de le recevoir du script d'installation. Qui installait sur un PC ordinaire se retrouvait avec un service qui échouait à chaque démarrage sans raison apparente. Nous ne l'avons vu qu'en démarrant vraiment une machine qui n'est pas une BC-250 : compiler pour elle ne suffisait pas."
    },
  },
  {
    data: '2026-08-23',
