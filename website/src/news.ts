...
export const news: Post[] = [
  {
    ...
    testo: {
      ...
      en: "Some flatpak entries used to sit in the update list looking as if they could never update. Nothing was failing to download: two identical-looking rows were two different versions of the same component, and system components no application uses any more came back at every check. The version now appears next to the name, what nothing depends on is removed after the update, and the list is read afresh instead of trusting the copy kept on the machine.",
